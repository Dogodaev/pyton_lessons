...
def bubbles_sort (scoress):
    swapped = True
    while swapped: # Цикл работает пока переменная swapped = True
        swapped = False
        for i in range(0, len(scoress)-1): #Цикл переставляет смежные элементы
            if scores[i] < scoress[i+1]:
                # Меняем значение в переданном листе
                temp = scoress[i]
                scoress[i] = scoress[i+1]
                scoress[i+1] = temp
                # Меняем значения индексов у переданного листа
...
# Объявляем список, в который запишем отсортированные значения
scores_sort = []

# Объявляем список с индексами сортируемого списка
solution_numbers =list(range(length))

# Обращаемся к объекту bubbles_sort и передаем ему массив scores
scores_sort = bubbles_sort(scores)

# Выводим ТОП 5 результатов
print()
print('Топ 5 растворов:')
for i in range(0,5):

    print('Пузырьковый раствор №'+ str(solution_numbers[i]),'- результат:',scores_sort[i])
